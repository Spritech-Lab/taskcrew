...
function planPrompt(card: Card): string {
  return [
    ...
    '',
    '### plan 的最後必須有一節「驗收條件的漏洞」',
    '',
    '驗收條件是人寫的，而**沒有任何東西在檢查那個人**。你是唯一同時看得到',
    '需求、驗收條件、和 codebase 實際結構的角色，所以這件事只有你做得到。',
    '',
    '要回答的是：**照這些條件做到全綠，會不會仍然是錯的東西？**',
    '',
    '- 有沒有明顯該驗但沒驗的行為（尤其是「錯了會無聲失敗」那種）',
    '- 有沒有哪條可以用作弊的方式滿足（寫死回傳值、砍掉功能讓測試消失）',
    '- 條件之間有沒有互相矛盾',
    '',
    '沒有漏洞就寫「沒有發現漏洞」。**不要為了顯得有在思考而湊**。',
    '發現漏洞時照樣把 plan 寫完 —— 那一節是給人看的，不是停手的理由；',
    '除非漏洞大到讓整份 plan 沒有意義，那時才用 `BLOCKED:`。',
    '',
